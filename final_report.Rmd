...
An Extreme Gradient Boosting (XGB) model was fit using the `xgboost` packages in `caret`. Gradient Boosting is another ensemble method for classification trees in which multiple trees are grown and used to create a more predictive model. It differs from other ensemble methods such as random forest and bagging because the algorithm focuses on modelling errors from previous models. We selected the `xgboost` algorithm because it performs more efficiently than the `gbm` package discussed in class.
