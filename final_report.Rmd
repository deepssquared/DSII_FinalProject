...
The method of K-nearest neighbors makes no assumptions about the shape of the decision boundary; therefore, if the decision boundary is highly non-linear then this model will perform better than others. This model has one tuning parameter, K. An optimal value of K was chosen by cross-validation using the `train()` function in the `caret package` by specifying the tuning grid.
...
Based off of our resampling results through the `caret` package, the best model to predict gender was the XGBoost model. 
